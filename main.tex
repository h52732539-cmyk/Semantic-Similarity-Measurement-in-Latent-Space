...
Code is available at \url{https://github.com/h52732539-cmyk/NLP}.
\end{abstract}
...
\paragraph{Summary.}
The results, illustrated in \Cref{fig:delta}, highlight a fundamental dichotomy:
\emph{structural} improvements (trained hyperbolic projection) yield consistent
cross-task gains, while \emph{heuristic} improvements (MaxSim, Span extraction)
are task- and mode-specific.
...
\Cref{tab:code-mapping} provides a quick reference for code review, linking each result in the paper to the script that produced it.
